...
// A XRT instance/adapter/device defined in this module with weak linkage.
WEAK void *global_instance = nullptr;
WEAK void *global_adapter = nullptr;
...
WEAK int halide_xrt_device_release(void *user_context) {
    ...
    err = halide_xrt_acquire_context(user_context,
...
WEAK int halide_xrt_initialize_kernels(void *user_context, void **state_ptr, const char *kernel_name) {
    ...
    snprintf(xclbin_name, sizeof(xclbin_name), "%s.xclbin", kernel_name);
    ...
    snprintf(open_name, sizeof(open_name), "%s:{%s_1}", kernel_name, kernel_name);
    kernel_handle = xrtPLKernelOpen(context.device, uuid, "toplevel");
    if (kernel_handle == XRT_NULL_HANDLE) {
        error(user_context)
            << "XRT: halide_xrt_initialize_kernels: "
            << "failed to open kernel: " << open_name << "\n";
        return halide_error_code_generic_error;
    }
